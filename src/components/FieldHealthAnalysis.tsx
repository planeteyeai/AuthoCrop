...
export const FieldHealthAnalysis: React.FC<FieldHealthAnalysisProps> = ({ fieldAnalysisData }) => {
  // Use computed status from overallHealth
  const percent = fieldAnalysisData?.overallHealth ?? 0;
  const computedStatus = getStatusLabel(percent);
  ...
  const arcColor = percent >= 85 ? '#16a34a' : '#f59e0b'; // green if healthy
  const remainingColor = percent >= 85 ? '#f59e0b' : '#16a34a'; // orange otherwise

  return (
    <div className="card h-full flex flex-col relative p-4 min-h-[320px]">
      <div className="flex justify-between items-start">
        <h2 className="card-title text-lg font-semibold text-gray-800">Field Score</h2>
        {fieldAnalysisData && (
          <div className="text-right text-sm text-gray-700">
            <div className="font-medium">PlotID : {fieldAnalysisData.plotName}</div>
          </div>
        )}
      </div>
