  ...
  return (
    <>
      <div className={`rounded-xl shadow-lg border-2 ${styles.bgColor} p-3 sm:p-4 md:p-6`}>
        <div className="flex flex-col sm:flex-row justify-between gap-3">
          <div className="flex-1">
            <h2 className="text-lg sm:text-xl font-bold text-black mb-2">
              {type === 'pest' ? 'Pest Detected' : 'Disease Detected'}: {name}
            </h2>
          ...
          <div className="relative flex-shrink-0">
            <img
              src={image}
              alt={name}
              className="w-24 h-32 sm:w-32 sm:h-40 object-cover rounded-md cursor-pointer"
        ...
        {isExpanded && (
          <div className="mt-4 text-xs sm:text-sm bg-white border border-gray-300 rounded-md p-3">
            <p className="font-bold text-black">Symptoms:</p>
            <ul className="list-disc pl-5">
              {symptoms.map((s: string, i: number) => <li key={i}>{s}</li>)}
            </ul>
            {type === 'pest' && identification.length > 0 && (
              <>
                <p className="mt-2 font-bold text-black">Identification:</p>
                <ul className="list-disc pl-5">
        ...
        <button
          className={`mt-2 flex items-center text-xs sm:text-sm font-medium ${styles.accentColor}`}
          onClick={onExpand}
        >
